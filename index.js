...
/* 	GÖREV 3:
  ...
  Bu diziyi kullanarak aşağıdakileri uygulayın:
		3a. enbuyuk ve enkucuk isminde 2 adet değişken tanımlayın ve sayilar dizisindeki en küçük sayı ile 
    en büyük sayıyı bu değişkenlere atayın. (for döngüsü kullanın)
...
let siralisayilar = [...besyuzdenkucuksayilar]
...
const sayilarinTekrarSayisi = {};

sayilar.forEach((sayi) => {
  if (sayilarinTekrarSayisi[sayi]) {
    sayilarinTekrarSayisi[sayi]++;
  } else {
    sayilarinTekrarSayisi[sayi] = 1;
  }
});

for (const sayi in sayilarinTekrarSayisi) {
  if (sayilarinTekrarSayisi[sayi] > 1) {
    const tekrarString = `${sayi} sayısı ${sayilarinTekrarSayisi[sayi]} kere tekrar edilmiştir`;
